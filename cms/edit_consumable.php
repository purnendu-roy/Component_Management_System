...
<body>
<form action="edit_consumable.php" method="POST" onLoad="_top" name="myForm" id="myForm">
<?php
...
{
	print "<br/><h3 align='center' style='color:red'><b>Select Consumable Items</b> </h3>";
   	addform();
}
?>
<?php
...
function searchData()
{
	...
	else
	{
		...
		echo "</th><th>Quantity</th><th>Price(Rs.)</th><th>Purchase Date</th>
		<th>Manufacturer</th><th>Bill No.</th><th></th></tr>";
	...
	while($row = mysql_fetch_array($result))
	{
	   ...
	   echo "<tr class='alt'><td>".$row[1]."</td><td>".$row[2]."</td><td>".$row[3]."</td><td>".$row[4]."</td>";
		...
		echo "<td>".$row[5]."</td><td>".$row[6]."</td><td>".$row[7]."</td><td>".$row[11]."</td><td>".$row[9]."
		</td><td><a href='edit_consumable_item.php?id=".$row['id']."'>Edit</a></td></tr>";
		}
		else
		{
			...
			if(!empty($row1['f10']))
				echo "<td>".$row['f10']."</td>";
echo "<td>".$row[5]."</td><td>".$row[6]."</td><td>".$row[7]."</td><td>".$row[11]."</td><td>".$row[9]."</td>
	<td><a href='edit_consumable_item.php?id=".$row['id']."'>Edit</a></td></tr>";
		}
		$v++;
	}
	echo "</table>";
