...
$gname=$row2['name'];
$sql="SELECT distinct uid FROM request_issue WHERE utype='Staff' and gname='$gname' and status='pending'";
...
{
	print "<br/><h3 style='color:grey' align='center'>Issue Requests</h5>";
	echo "<table border='1' align='center' id='edittab'><tr><th>No</th><th>Staff ID</th><th>Staff Name</th></tr>";
	...
	{
		$rno=$row['uid'];
		$sql1="SELECT * FROM staff WHERE id='$rno'";
		$result1=mysql_query($sql1);
		$row1=mysql_fetch_array($result1);
		if(($v%2)==0)
		{	
		  echo "<tr class='alt'><td>".$no."</td><td>
		  <a href='staff_issue_request_details.php?rno=".$row['uid']."&name=".$row1['name']."'>".$row['uid'].
		  "</a></td><td>".$row1['name']."</td></tr>";
		}
		else
		{
		  echo "<tr><td>".$no."</td><td>
		  <a href='staff_issue_request_details.php?rno=".$row['uid']."&name=".$row1['name']."'><u>".$row['uid'].
		  "</u></a></td><td>".$row1['name']."</td></tr>";
		}
	$v++;
	$no++;
	}
