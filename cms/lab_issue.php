...
<body>
<form action="lab_issue.php" method="POST" onLoad="_top">
<?php
include 'admin.php';
...
else if($_POST['rno'])
{
	...
	$uid=$row1['id'];
	$sql2="SELECT * FROM request_issue WHERE uid='$uid' and status='Approved'";
	$result2=mysql_query($sql2);
	
	if(mysql_num_rows($result2) == 0)
	{
			print "<br/><h3 style='color:red' align='center'>No Issue Requests</h3><center/>";
			echo "<br><button type='button' onclick='history.back();'>Back</button>";
	}
	else
		header("location:lab_issue_details.php?id=$uid");
}
else
{
