...
<body>
<form action="staff_issue_request_details.php" method="POST" onLoad="_top">
<?php
include 'faculty.php';
if(isset($_POST['dapproval']))
{
	dapprove();
}
else if(!isset($_POST['approval']))
{
	$id=$_GET["rno"];
	$name=$_GET["name"];
	print "<h3 align='center' style='color:grey'>Issue requests by ".$name."</h3>";
	form($id,$name);
}
else if($_POST['approval'])
{
	approve();
}
else
{
	print "<br/><h3 style='color:red' align='center'>eeeeeeeeeeeeeej</h3>";
}
?>
<?php
function form($id,$name)
{
	...
	$gname=$row2['name'];
	echo "<table border='1' align='center' id='edittab'><tr><th>No</th><th>Component</th><th>Category</th>
	<th>Sub Category</th><th>Description</th><th>Quantity</th><th>Request Date</th><th></th></tr>";
	
	$no=1;
	$v=1;
	
	$sql="SELECT * FROM request_issue WHERE uid='$id' and utype='Staff' and gname='$gname' and status='pending'";
	...
	while($row = mysql_fetch_array($result))
	{
		...
		if(($v%2)==0)
		{
			...
			<u>".$row3['type']."</u></a></td><td>".$row3[2]."</td><td>".$row3[3]."</td><td>".$row3[4]."</td>
			<td><u><a href='edit_quantity.php?cid=$row[0]&id=$row[1]&name=$name&ut=Staff'>".$row['quantity'].
			"</a></u></td><td>".$row['rdate']."</td><td><input type='checkbox' name='items[]' value='".$id."'>	
			</input></td></tr>";
		}
		else
		{
			...
			<u>".$row3['type']."</u></a></td><td>".$row3[2]."</td><td>".$row3[3]."</td><td>".$row3[4]."</td>
			<td><u><a href='edit_quantity.php?cid=$row[0]&id=$row[1]&name=$name&ut=Staff'>".$row['quantity']."</a>
			</u></td><td>".$row['rdate']."</td><td><input type='checkbox' name='items[]' value='".$id."'></input>
			</td></tr>";
		}
	$v++;
	$no++;
	...
	echo "<tr><td><input type='submit' name='approval' value='Approve Selected'></td>
	<td></input><input type='submit' name='dapproval' value='Disapprove Selected'></input></td>
...
function approve()
{
	if(isset($_POST['items']))
	{
		...
		else
		{
			...
			echo "<h3 style='color:red' align='center'>Requests Approved Successfully </h3>";
			echo "<br/><br/><center/><button type='button' onclick='history.back();'>Back</button>";
		}
	}
	else
	{
		...
		echo "<br/><br/><center/><button type='button' onclick='history.back();'>Back</button>";
		//echo "<script>alert('You Didn't Select any items')</script>";
	}
}
function dapprove()
{
