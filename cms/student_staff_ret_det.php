...
<body>
<form action="student_staff_ret_det.php" method="POST" onLoad="_top">
...
function form($id)
{
print "<br/><h3 style='color:red' align='center'>Components issued to ".$id."</h3>";
echo "<br/><table border='1' align='center' id='edittab'><tr><th>No.</th><th>Component</th><th>Category</th>
<th>Sub Category</th><th>Description</th><th>Quantity</th><th>Issue Date</th><th>Return Date</th><th></th></tr>";
...
$sql="select * from lab_det l, user_issue_consumable u where u.uid='$id' and l.staff_id='$stid' and 
l.fac_in=u.guide ";
...
while($row = mysql_fetch_array($result))
{
	...
	if(($v%2)==0)
	{
		...
		<td>".$row['quantity']."</td><td>".$row['idate']."</td><td>".$row['rdate']."</td>
		<td><input type='checkbox' name='items[]' value='".$row['id']."'></input></td></tr>";
	}
	else
	{
...
<td><input type='submit' name='allreturn' value='Return All'></input></td></tr></table>";
}
...
function cmpreturn()
{
	if(isset($_POST['items']))
	{
		...
		else
		{
			...
			for($i=0;$i<$N;$i++)
			{
				...
				$sql="UPDATE component SET quantity='$quant' where id='$cid'";
				$result=mysql_query($sql);
	...
	print "<br/><h3 style='color:red' align='center'>Selected components returned successfully</h5>";
...
function returnall()
{
	...
	print "<br/><h3 style='color:red' align='center'>All components returned successfully</h5>";
	//header("refresh:2,staff.php");
}
?>
</form>
